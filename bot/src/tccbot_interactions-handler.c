...
CCORDcode tccbot_handle_ask(struct discord *client, const struct discord_interaction *event, struct discord_interaction_response *params)
{
    ...
    if (event->data->options)
    {
        ...
        {
            const struct discord_application_command_interaction_data_option *option = &event->data->options->array[i];
                    ...
                    .array = (struct discord_component[]){
                        {
                            ...
                            .custom_id = anonymous ? "anonymous_question_input" : "question_input",
                            .label = anonymous ? "Your Anonymous Question" : "Your Question",
    ...
    return discord_create_interaction_response(client, event->id, event->token, params, NULL);
}
...
CCORDcode tccbot_handle_ask_ack(struct discord *client, const struct discord_interaction *event, struct discord_interaction_response *params)
{
    ...
    cog_u64tostr(join_time, sizeof(join_time), &(uint64_t){cog_timestamp_ms()});
    ...
    {
        const struct discord_components *modals = event->data->components->array[i].components;
        if (!modals || modals->size == 0)
            continue;

        for (int j = 0; j < modals->size; ++j)
        {
            if (strcmp(modals->array[j].custom_id, "anonymous_question_input") == 0)
    ...
    {
        jsonb_key(&ctx->serial.b, payload, sizeof(payload), "studentId", sizeof("studentId") - 1);
        jsonb_string(&ctx->serial.b, payload, sizeof(payload), student_id, strlen(student_id));
        jsonb_key(&ctx->serial.b, payload, sizeof(payload), "question", sizeof("question") - 1);
        jsonb_string(&ctx->serial.b, payload, sizeof(payload), question, strlen(question));
        jsonb_key(&ctx->serial.b, payload, sizeof(payload), "timestamp", sizeof("timestamp") - 1);
        jsonb_string(&ctx->serial.b, payload, sizeof(payload), join_time, strlen(join_time));
        jsonb_key(&ctx->serial.b, payload, sizeof(payload), "username", sizeof("username") - 1);
        jsonb_string(&ctx->serial.b, payload, sizeof(payload), username, strlen(username));
    }
...
CCORDcode tccbot_handle_help(struct discord *client, const struct discord_interaction *event, struct discord_interaction_response *params)
{
    ...
    params->data->content = "🤖 **tccbot - Educational Assistant**\n\n"
                            "**Commands:**\n"
...
CCORDcode tccbot_handle_role_select(struct discord *client, const struct discord_interaction *event, struct discord_interaction_response *params)
{
    for (int i = 0; i < event->data->values->size; ++i)
    {
        ...
        if (!str_role_id || !*str_role_id)
            continue;

        u64snowflake role_id = 0ULL;
        cog_strtou64((char *)str_role_id, strlen(str_role_id), &role_id);
        discord_add_guild_member_role(client, event->guild_id, event->member->user->id, role_id, NULL, NULL);
    }
        ...
        .array = (struct discord_component[]){
            {
                ...
                .components = &(struct discord_components){
                    ...
                    .array = (struct discord_component[]){
                        {
                            .type = DISCORD_COMPONENT_TEXT_INPUT,
                            .custom_id = anonymous ? "anonymous_answer_input" : "answer_input",
...
CCORDcode tccbot_handle_answer_ack(struct discord *client, const struct discord_interaction *event, struct discord_interaction_response *params)
{
    ...
    for (int i = 0; i < event->data->components->size; ++i)
    {
        ...
        for (int j = 0; j < modals->size; ++j)
        {
            if (strcmp(modals->array[j].custom_id, "anonymous_answer_input") == 0)
    ...
    jsonb_object(&ctx->serial.b, payload, sizeof(payload));
    {
        jsonb_key(&ctx->serial.b, payload, sizeof(payload), "studentId", sizeof("studentId") - 1);
        jsonb_string(&ctx->serial.b, payload, sizeof(payload), student_id, strlen(student_id));
        jsonb_key(&ctx->serial.b, payload, sizeof(payload), "answer", sizeof("answer") - 1);
        jsonb_string(&ctx->serial.b, payload, sizeof(payload), answer, strlen(answer));
        jsonb_key(&ctx->serial.b, payload, sizeof(payload), "timestamp", sizeof("timestamp") - 1);
        jsonb_string(&ctx->serial.b, payload, sizeof(payload), join_time, strlen(join_time));
        jsonb_key(&ctx->serial.b, payload, sizeof(payload), "username", sizeof("username") - 1);
        jsonb_string(&ctx->serial.b, payload, sizeof(payload), username, strlen(username));
