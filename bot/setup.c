...
static int
tccbot_save_roles_to_file(struct discord_roles *roles)
{
...
static int
tccbot_create_commands(struct discord *client, struct tccbot_context *ctx)
{
    CCORDcode code;

    for (size_t i = 0; i < sizeof(cmds) / sizeof(*cmds); ++i) {
        code = discord_create_guild_application_command(
            client, ctx->setup.application_id, ctx->setup.guild_id, &cmds[i],
    ...
    logmod_log(INFO, NULL,
               "Application commands created successfully in guild %" PRIu64,
               ctx->setup.guild_id);
    return 1;
}
...
static struct discord_create_message menu = {
    .content =
        "🎓 **Welcome to TCC Bot!**\n\nPlease select your class code below to "
        "join the class and get access to your class-specific channels:",
    .components =
...
static int
tccbot_create_menu(struct discord *client, struct tccbot_context *ctx)
{
    char **class_codes = ctx->setup.class_codes.array;
    const size_t num_classes = ctx->setup.class_codes.size;
    const u64bitmask default_permissions = DISCORD_PERM_READ_MESSAGE_HISTORY
    ...
    code =
        discord_get_guild_roles(client, ctx->setup.guild_id,
                                &(struct discord_ret_roles){ .sync = &roles });
    if (code == CCORD_OK && roles.size > 0) {
        for (size_t i = 0; i < num_classes; ++i) {
            for (int j = 0; j < roles.size; ++j) {
                if (strcmp(roles.array[j].name, class_codes[i]) != 0) continue;

                logmod_log(INFO, NULL,
                           "Deleting existing role: %s (%" PRIu64 ")",
                           roles.array[j].name, roles.array[j].id);
                CCORDcode code = discord_delete_guild_role(
                    client, ctx->setup.guild_id, roles.array[j].id, NULL,
                    &(struct discord_ret){
                        .sync = true,
                    });
                if (code != CCORD_OK)
                    logmod_log(ERROR, NULL,
                               "Failed to delete existing role: %s",
    ...
    for (size_t i = 0; i < num_classes; ++i) {
        code = discord_create_guild_role(
        ...
        if (code != CCORD_OK) {
            logmod_log(ERROR, NULL,
                       "Failed to create class selection role: %s",
                       discord_strerror(code, client));
            return discord_roles_cleanup(&roles), 0;
        }
        logmod_log(INFO, NULL, "Created role: %s (%" PRIu64 ")",
                   class_codes[i], roles.array[i].id);
    }

    // Save role IDs to roles.json
    if (!tccbot_save_roles_to_file(&roles)) {
    ...
    code = discord_create_message(
        client, ctx->setup.rules_channel_id, &menu,
        &(struct discord_ret_message){ .sync = DISCORD_SYNC_FLAG });
    if (code != CCORD_OK) {
        logmod_log(ERROR, NULL, "Failed to create menu: %s",
                   discord_strerror(code, client));
        return 0;
    }
    logmod_log(INFO, NULL,
               "Class selection menu created successfully in channel %" PRIu64,
               ctx->setup.rules_channel_id);
    return 1;
}
